...
//        }
        loginManager.logIn(readPermissions: [.publicProfile, .email], viewController: viewController) { loginResult in
			...
			case .success:
				SVProgressHUD.show()
				FBSDKGraphRequest(graphPath: "me", parameters: ["fields": "email,id,first_name,last_name,gender"]).start(completionHandler: { (connection, result, error) -> Void in
					if (error == nil) {
...
//                                "data[api_key]": DefineMacros.API_KEY,
								...
								"data[User][picture]": "https://graph.facebook.com/" + facebookId + "/picture?width=500&height=500",
